...
def listen_once(*, timeout=None):
    ...
    with http.server.HTTPServer(
        ("127.0.0.1", 10043), CompetitiveCompanionHandler
    ) as server:
    ...
    if "USACO" in data["group"]:
        if "fileName" in data["input"]:
            names = [
                ...
                data["output"]["fileName"].rstrip(".output"),
            ]
            if len(set(names)) == 1:
                return names[0]
...
def save_samples(data, prob_dir):
    ...
    # stores information in README.md
    with open(prob_dir / "README.md", "w") as f:
        f.write(f"# [{data['name']}]({data['url']})\n\n")
        f.write(f"**Group:** {data['group']}\n\n")
        f.write(f"**URL:** {data['url']}\n\n")
        f.write(f"**Interactive:** {'Yes' if data['interactive'] else 'No'}\n\n")
        f.write(f"**Memory Limit:** {data['memoryLimit']} MB\n\n")
        f.write(f"**Time Limit:** {data['timeLimit']} ms\n\n")
        f.write("## Tests\n")
    ...
    for i, t in enumerate(data["tests"], start=1):
        with open(prob_dir / f"sample{i}.in", "w") as f:
            f.write(t["input"])
        with open(prob_dir / f"sample{i}.output", "w") as f:
            f.write(t["output"])
...
def make_prob(data, name=None):
    ...
    if name == ".":
        ...
        pass
    elif prob_dir.exists() and prob_dir.is_dir() and (prob_dir / name).exists():
        print(f"Already created problem {name}...")
    else:
        print(f"Creating problem {name}...")

        MAKE_PROB = Path("~/cplib/utils/gen").expanduser()
        prob_dir.mkdir(exist_ok=True)
        try:
            subprocess.check_call(
                [str(MAKE_PROB), "-o", prob_dir / name],
                stdout=sys.stdout,
                stderr=sys.stderr,
            )
